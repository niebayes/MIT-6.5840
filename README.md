...
岗位职责：
1、参与设计和开发存算分离数据库，参与整个项目生命周期；
2、按照任务需要实现部分核心代码及其相关 UT；
3、根据项目需要，对特定领域的优化进行研究；

岗位要求：
1、熟悉 C++ 编程语言，有过 Rust 项目经验的优先考虑；
2、了解至少一种主流数据库的源代码，能够清晰描述其实现原理（如 MySQL、Clickhouse 等）；
3、有开源项目贡献经验优先；
4、熟悉 Clickhouse、FoundationDB、RocksDB 优先；

薪资福利：
Base 成都，250-300 元/天，不定期团建，技术氛围，成长空间，优秀者可转正
